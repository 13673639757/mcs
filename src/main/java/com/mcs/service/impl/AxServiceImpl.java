...
public class AxServiceImpl implements AxService {
    private Logger logger = Logger.getLogger(AxServiceImpl.class);
    private String appKey="dGkohXk54Q3RRB4QCSZYE00dK3R9";
    private String appSecret="n3lK73gix7UXcuD1Oy4qxZXNAkuc";
    private String ompDomainName="https://rtcapi.cn-north-1.myhuaweicloud.com:12543";
    ...
    @Override
    public String axBindNumber(String origNum, String privateNum, String calleeNumDisplay) {
        // 必填,AX模式绑定接口访问URI
        ...
        json.put("origNum", "+86"+origNum); // A方真实号码(手机或固话)
        json.put("privateNum","+86"+privateNum); // 已订购的隐私号码(X号码)
...
//        json.put("preVoice", preVoice); //个性化通话前等待音
    ...
    @Override
    public String axUnbindNumber(String subscriptionId, String origNum, String privateNum) {
        // 必填,AX模式解绑接口访问URI
        String url = "/rest/provision/caas/privatenumber/v1.0";
        String realUrl = buildOmpUrl(url);
        ...
        if (StringUtils.isNotBlank(origNum)) {
            map.put("origNum", "+86"+origNum); // AX中的A号码
        }
        if (StringUtils.isNotBlank(privateNum)) {
            map.put("privateNum", "+86"+privateNum); // AX中的X号码
        }
